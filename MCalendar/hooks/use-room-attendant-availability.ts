...
const KEY = "room-attendant-availability";
  ...
  return useQuery({
    ...
    queryFn: () =>
      api.get<Paginated<RoomAttendantAvailabilityView>>(
        `/api/room-attendant-availability?${query.toString()}`,
      ),
  });
}
...
export function useCreateAvailability() {
  ...
  return useMutation({
    mutationFn: (body: CreateAvailabilityDTO) =>
      api.post<RoomAttendantAvailabilityView>("/api/room-attendant-availability", body),
...
//       api.patch<RoomAttendantAvailabilityView>(`/api/room-attendant-availability/${id}`, body),
//     onSuccess: () => qc.invalidateQueries({ queryKey: [KEY] }),
//   });
// }
...
export function useDeleteAvailability() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (id: string) => api.del(`/api/room-attendant-availability/${id}`),
    onSuccess: () => qc.invalidateQueries({ queryKey: [KEY] }),
  });
}
