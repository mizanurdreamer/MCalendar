...
import { updateRoomAttendantProfileSchema, type UpdateRoomAttendantProfileDTO } from "@/dto/profile.dto";
import { useRoomAttendantProfile, useUpdateRoomAttendantProfile } from "@/hooks/use-profiles";
...
export function RoomAttendantProfileSection() {
  const [editing, setEditing] = React.useState(false);
  const { data, isLoading } = useRoomAttendantProfile();
  const update = useUpdateRoomAttendantProfile();

  const { register, handleSubmit, reset, formState: { errors } } = useForm<UpdateRoomAttendantProfileDTO>({
    resolver: zodResolver(updateRoomAttendantProfileSchema),
    values: {
  ...
  return (
    <div className="space-y-5">
      ...
      <Card className="rounded-2xl">
        <CardHeader>
          <CardTitle className="text-lg ">RoomAttendantProfile</CardTitle>
        </CardHeader>
        <CardContent className="grid gap-4 md:grid-cols-2">
