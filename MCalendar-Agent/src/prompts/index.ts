...
export function getSystemPrompts(projectName: string): Record<string, string> {
  return {
    agent_issue_analyzer: replaceProjectName(AGENT_ISSUE_ANALYZER_PROMPT, projectName),
    agent_commit_analyzer: AGENT_COMMIT_ANALYZER_PROMPT,
    agent_tests_generator: replaceProjectName(AGENT_TESTS_GENERATOR_PROMPT, projectName),
    agent_tests_reviewer: AGENT_TESTS_REVIEWER_PROMPT,
    agent_tests_report_generator: AGENT_TESTS_REPORT_GENERATOR_PROMPT,
    agent_summarize: replaceProjectName(AGENT_SUMMARIZE_PROMPT, projectName),
  };
}
