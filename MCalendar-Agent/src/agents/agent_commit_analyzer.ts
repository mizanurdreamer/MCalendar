...
${fileList}`;
    ...
    try {
      ...
      if (cached) {
        ...
        logger.info(`[AgentCommitAnalyzer] Loaded cached project context`);
      } else {
        ...
        logger.info(`[AgentCommitAnalyzer] Project context discovered and saved`);
      }
    } catch (err) {
      logger.warn(`[AgentCommitAnalyzer] Project context save failed: ${err}`);
    }
