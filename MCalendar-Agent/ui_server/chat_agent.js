...
function buildTools(deps) {
    return [
        ...
        {
            name: "process_issue",
            description: "Start the full test-generation agent pipeline on a GitHub issue in the background " +
        ...
        {
            name: "process_commit",
            description: "Start the commit-analysis agent pipeline on a commit SHA in the background " +
...
async function executeTool(name, input, deps) {
    ...
    switch (name) {
        ...
        case "check_job_status": {
            ...
            if (!status.busy && status.history.length === 0)
                return "No jobs have been started yet. Agent idle.";
            const lines = [];
            if (status.current)
                lines.push(`RUNNING: ${status.current.label} (started ${new Date(status.current.startedAt).toLocaleTimeString()})`);
            else
                lines.push("No job currently running.");
...
function startJobInBackground(deps, label, start) {
...
export async function runChatTurn(userMessage, history, deps) {
    const { config } = deps;
